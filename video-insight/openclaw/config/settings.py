...
环境变量优先级（从低到高）：
1. YAML 配置文件
2. 标准 API key 环境变量（推荐）：
   - OPENAI_API_KEY
   - DEEPSEEK_API_KEY
   - VOLCENGINE_API_KEY  （豆包/火山引擎）
   - DASHSCOPE_API_KEY   （通义千问）
3. 完整路径环境变量（高级覆盖）：OPENCLAW__LLM_PROVIDERS__OPENAI__API_KEY=xxx
"""
...
def load_settings(config_path: str = "config.yaml") -> AppSettings:
    ...
    1. 模型字段默认值
    2. YAML 配置文件
    3. 标准 API key 环境变量（OPENAI_API_KEY、DEEPSEEK_API_KEY 等）
    4. 完整路径环境变量（OPENCLAW__LLM_PROVIDERS__OPENAI__API_KEY 等）
    5. .env 文件中的环境变量
    ...
    # 将标准 API key 环境变量映射到配置结构
    # 优先级：标准变量名 < OPENCLAW__ 前缀变量（后者由 pydantic-settings 自动处理）
    _apply_standard_env_vars(yaml_data)

    return AppSettings(**yaml_data)


# 标准环境变量名 → llm_providers 中的 provider key 映射
_STANDARD_ENV_MAP: Dict[str, str] = {
...
def _apply_standard_env_vars(yaml_data: dict) -> None:
    """将标准 API key 环境变量注入到 yaml_data 的 llm_providers 中。

    只在 YAML 中未设置（或为空）时才填入，OPENCLAW__ 前缀变量优先级更高，
    由 pydantic-settings 在构造时自动覆盖。

    特殊处理：
    - openclaw provider：无需 API key，自动激活；支持 OPENCLAW_GATEWAY_URL 覆盖端口
    """
    providers: dict = yaml_data.setdefault("llm_providers", {})

    # 标准 API key 映射
    for env_var, provider_key in _STANDARD_ENV_MAP.items():
        if not env_var:
            continue
    ...
    # openclaw gateway：始终激活，支持自定义端口
    gateway_url = os.environ.get("OPENCLAW_GATEWAY_URL", "").strip()
    openclaw_provider = providers.setdefault("openclaw", {})
    if not openclaw_provider.get("api_key"):
        openclaw_provider["api_key"] = "openclaw"  # 占位符，gateway 不校验
    if gateway_url and not openclaw_provider.get("base_url"):
        openclaw_provider["base_url"] = gateway_url.rstrip("/") + "/v1"
